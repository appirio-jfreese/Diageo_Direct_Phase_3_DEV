...
Date        : 11/1/13
Story/Task  : DE759
Changes     : Modified existing system.debug calls to include reference to this trigger and added new system.debug calls for Magento sync tracking
              Also added if(TriggerState.isActive) logic to de-activate/activate the trigger via the TriggerSettings custom setting

Updated By  : Jonathan Freese   
Date        : 12/12/13
Story/Task  : DE782
Changes     : Added Item_ID__c to the select query in order to pass it to Magento
...
trigger POS_Product_Item on Item__c (
    ...
    private static String itemsWithBrands = 'Select i.Id, i.Status__c, i.Program__r.Magento_Id__c, i.Program__r.Order_Window__r.Magento_Id__c, i.Order_Window_Name__c, i.Name, i.Current_Price__c,' +
         'i.Description__c, i.Estimated_Price__c, i.Category__c, i.Minimum_Quantity__c, i.Packout_Quantity__c, i.Item_Category__r.Name, i.Item_Category__r.Friendly_Name__c,' +
          /*Start: changes for DE782 jfreese*/
         'i.Item_ID__c,' +
          /*End: changes for DE782 jfreese*/
    ...
    try{
			maxnum = Integer.valueOf(Service_API.getSetting('maxNoItemsForFutureCall'));
		}catch(Exception e){
			maxnum = 0;
		}
    ...
    //Start: changes for DE759 jfreese
    if(TriggerState.isActive('POS_Product_Item')) { //de-activate/activate this entire trigger via custom_settings TriggerSettings
    //End: changes for DE759 jfreese
    ...
    if (trigger.isInsert) {
        ...
        //End: Changes for US861/TA1538 JFreese	
           ...
           for(Item__c n : trigger.new) {
                ...
                System.debug('POS_Product_Item.trigger n.Name -' +n.Name);
                System.debug('DE759 POS_Product_Item.trigger Item after initial insert: Item Name - ' + n.name + ', Item_ID - ' + n.Item_ID__c + ', Agency_Item_ID - ' + n.Agency_Item_ID__c + ', Magento_ID - ' + n.Magento_ID__c);
            }
            ...
            for(Item__c itm: itemsForSync) {
            	System.debug('POS_Product_Item.trigger after_insert calling syncMagentoProduct to create.product for item=' + itm.id + ', maxnum=' + maxnum);
                if(itemsForSync.size() > maxnum ) {
           ...
           for(Id itmId:itemProgamIdMap.keySet()) {
                System.debug('POS_Product_Item.trigger itmId - ' + itmId + ' itemProgamIdMap.get(itmId) - ' + itemProgamIdMap.get(itmId));
                Program__c prg = programMap.get(itemProgamIdMap.get(itmId));
                System.debug('POS_Product_Item.trigger prg.Brand__c -' + prg.Brand__c + ' prg.Brand__r.Portfolio_Brand__c -' + prg.Brand__r.Portfolio_Brand__c);
            ...
            System.debug('POS_Product_Item.trigger ibCreateList.size() -' + ibCreateList.size());
            insert  ibCreateList;           
        //Start: Changes for US861/TA1538 JFreese
        }
        //End: Changes for US861/TA1538 JFreese
           ...
           for(Item__c n : trigger.new) {
                ...
                //}
                System.debug('POS_Product_Item.trigger n.CreatedDate -' +n.CreatedDate + ' n.LastModifiedDate -' +n.LastModifiedDate);
                System.debug('DE759 POS_Product_Item.trigger isUpdate: Item Name - ' + n.name + ', Item_ID - ' + n.Item_ID__c + ', Agency_Item_ID - ' + n.Agency_Item_ID__c + ', Magento_ID - ' + n.Magento_ID__c);
            }
            ...
            for(Item__c itm: itemsForSync) {
                ...
                System.debug('POS_Product_Item.trigger itemsForSync.size() -' +itemsForSync.size());
                ...
                System.debug('POS_Product_Item.trigger trigger.newMap.get(itm.id).Magento_Option_Id__c =' + trigger.newMap.get(itm.id).Magento_Option_Id__c +
                    ' itm.Magento_Option_Id__c =' + itm.Magento_Option_Id__c);
                System.debug('POS_Product_Item.trigger after_update calling syncMagentoProduct to update.product for item=' + itm.id + ', Parent=' + itm.Parent__c);
                ...
                */
                else if(itemsForSync.size() > maxnum || Test.isRunningTest()) {
            ...
            if(parentItemIds.size() > 0) {
                ...
                System.debug('POS_Product_Item.trigger parentItemsForSync.size() -' +parentItemsForSync.size());
                
                for(Item__c pitm: parentItemsForSync) {
                    System.debug('POS_Product_Item.trigger after_update calling syncMagentoProduct to update.product for parent item=' + pitm.id);
           ...
           for(Id itmId:itemProgamIdMap.keySet()) {
            ...
            if(kitIds.size() > 0) {
                ...
                for(Item__c itm: itemsForKitCheck) {
                        System.debug('POS_Product_Item.trigger ----KitItem'+itm.Kit_Items__r);
            ...
            if(rollupIds.size() > 0) {
                ...
                for(Item__c itm: itemsForRollupCheck) {
                        ...
                        if(itm.Items__r.size() < 1) {
                            ...
                            tItm.addError('POS_Product_Item.trigger Roll-Up Item must contain at least one Child items while in status Approved or Accepted');
                        }                   
                }   
            }   
           ...
           for(Item__c o : trigger.old) {
                ...
                System.debug('POS_Product_Item.trigger o.Magento_Id__c -' +o.Magento_Id__c);
            ...
            if(parentItemIds.size() > 0) {
                List<Item__c> parentItemsForSync  = Database.query(itemsWithBrands  + ' :parentItemIds');
                System.debug('POS_Product_Item.trigger Delete parentItemsForSync.size() -' +parentItemsForSync.size());
                
                for(Item__c pitm: parentItemsForSync) {
                    System.debug('POS_Product_Item.trigger after_delete calling syncMagentoProduct to update.product for parent item=' + pitm.id);
                    POS_MagentoProduct.syncMagentoProduct(pitm, 'update', false);  // rollup parent always route through Queue
                }
            }       
