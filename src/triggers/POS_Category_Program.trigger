...
trigger POS_Category_Program on Program__c (after delete, after insert, after update) {
    ...
    if(TriggerState.isActive('POS_Category_Program')) { //de-activate/activate this entire trigger via custom_settings TriggerSettings
    //End: changes for US278/TA1627 jfreese
	   ...
	   if (trigger.isDelete) {
		   for(Program__c o : trigger.old) {
				...
				POS_MagentoCategory.deleteMagentoCategory(o.Magento_Id__c, 'Program__c');
                //End: changes for US278/TA1627 jfreese
		   ...
		   for(Program__c n : trigger.new) {
				...
				if(o.Magento_Id__c != null) { // donot sync when unpdating the Magento Id back on the record after creation
					...
					if(!Test.isRunningTest()) { // avoid futureCallouts
						...
						POS_MagentoCategory.updateMagentoCategoryProgram(n.Magento_Id__c, n.Name, n.Sales_Channel_Group__c, n.Sales_Driver__c, isActive, n.Id);
                        //End: changes for US278/TA1627 jfreese
					}
				}
