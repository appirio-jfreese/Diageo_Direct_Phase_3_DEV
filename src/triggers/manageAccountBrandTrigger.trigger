...
trigger manageAccountBrandTrigger on Brand__c (after insert, after delete) {
    ...
    if(trigger.isInsert){
        ...
        List<Brand__c> childBrands = [SELECT Id FROM Brand__c WHERE L3_Brand__c IN :brandIDs AND Brand_Type__c = 'Individual Variant' AND Id NOT IN :brandIDs];
/* US817 RKW 11/18/2013        List<Brand__c> childBrands = [SELECT Id FROM Brand__c WHERE Parent_Brand__c IN :brandIDs AND Id NOT IN :brandIDs];  */
