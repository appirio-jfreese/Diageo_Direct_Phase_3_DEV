...
Date        : 12/18/13
Story/Task  : US543
Changes     : Added if(TriggerState.isActive) logic to de-activate/activate the trigger via the TriggerSettings custom setting

**************************************************************************************************/
trigger POS_Budget on Budget__c (before insert, before update, after insert, after update, after delete) {
    
    //Start: changes for US543 jfreese
    if(TriggerState.isActive('POS_Budget_Trigger')) { //de-activate/activate this entire trigger via custom_settings TriggerSettings
    //End: changes for US543 jfreese
        ...
        for(Budget__c n : trigger.new) {
            ...
            if(n.Brand__c != null) { // Budget Allocation by Brand, check for Shopper, Brand combination to be valid
                ...
                if(!shopperBrandValid) {
                    ...
                    if(iBrandsMap.containsKey(n.Order_Window__c)){
                    ...
                    List<Item_Brand__c> iParentBrands = new List<Item_Brand__c>();
                    if(iParentBrandsMap != null && iParentBrandsMap.containsKey(n.Order_Window__c)){
                    	iParentBrands = iParentBrandsMap.get(n.Order_Window__c);
                    }
                    //4.22.2013-CASE 4878 - LSLEVIN - START//
