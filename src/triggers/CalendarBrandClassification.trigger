...
trigger CalendarBrandClassification on CalendarBrand__c (after insert, after update) {
	if(Trigger.isAfter && Trigger.isInsert){
		...
		CalenderBrandTriggerHandler.onAfterInsert(Trigger.new);
	}
		
	if(Trigger.isAfter && Trigger.isUpdate){
		CalenderBrandTriggerHandler.onAfterUpdate(Trigger.new, Trigger.oldMap);
	}
}